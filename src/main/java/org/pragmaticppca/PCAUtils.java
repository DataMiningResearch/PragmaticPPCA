...
class PCAUtils {
	...
	*/
	...
	public static void outerProductWithMissIndices(org.apache.spark.mllib.linalg.Vector yObsi, 
			org.apache.spark.mllib.linalg.Vector yMissi, 
			Vector ym, double[] xi,
			Vector xm, double[][] resArray, int[] nonZeroIndicesObs, int[] nonZeroIndicesMiss) {
		// 1. Sum(Yi' x (Xi-Xm))
		
		int xSize = xi.length;
		double yScale=0;
		int i, yRow;
		for(i=0; i < nonZeroIndicesObs.length; i++)
		{
			yRow=nonZeroIndicesObs[i];
			yScale=yObsi.apply(yRow);
			for (int xCol = 0; xCol < xSize; xCol++) {
				double centeredValue = xi[xCol] - xm.getQuick(xCol);
				resArray[yRow][xCol] += centeredValue * yScale;
			}
		...
		for(i=0; i < nonZeroIndicesMiss.length; i++)
		{
			yRow=nonZeroIndicesMiss[i];
			yScale=yMissi.apply(yRow);
			for (int xCol = 0; xCol < xSize; xCol++) {
				double centeredValue = xi[xCol] - xm.getQuick(xCol);
				resArray[yRow][xCol] += centeredValue * yScale;
			}
	...
	static void sparseMissVectorTimesMatrix(org.apache.spark.mllib.linalg.Vector sparseObsVector,
			org.apache.spark.mllib.linalg.Vector sparseMissVector, Matrix matrix,
			double[] resArray) {
		int matrixCols = matrix.numCols();
		...
		for (int col = 0; col < matrixCols; col++) 
		{
			...
			{
				index=indicesMiss[i];
				value=sparseMissVector.apply(index);
				dotRes += matrix.getQuick(index,col) * value;
			}
			resArray[col] = dotRes;
		}
	...
	static org.apache.spark.mllib.linalg.Vector 
	 reconMissValue(org.apache.spark.mllib.linalg.Vector sparseMissVector, Vector ym, double[] resArray,
			 Vector xm,	 Matrix matrix) {
		int matrixCols = matrix.numCols();
		int[] indicesMiss;
		indicesMiss=((SparseVector)sparseMissVector).indices();
	...
	*/
	...
	public static void printMatrixToFile(org.apache.spark.mllib.linalg.Matrix m, OutputFormat format, String outputPath
			,String name) {
		String outputFilePath=outputPath+ File.separator + name;
		switch(format)
		{
			case DENSE:
