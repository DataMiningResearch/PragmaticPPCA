...
public class ReconError implements Serializable {
	...
	public static int nClusters = 320;
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		try {
			...
			log.warn("Default name for dataset " + dataset + " will be used ");
		}
		...
		computePrincipalComponents(sc, inputPath, inputPathV, outputPath, nRows, nCols, nPCs, subsample, precision,
				k_plus_one_singular_value, maxIterations);
	...
	public static org.apache.spark.mllib.linalg.Matrix computePrincipalComponents(JavaSparkContext sc, String inputPath,
			...
			final double precision, final double k_plus_one_singular_value, final int maxIterations) throws FileNotFoundException {
		...
		spectral_error = norm(sc, vectors, nRows, nCols, 1, subsample, maxIterations, meanVector, V,br_ym_mahout, precision);
		...
		for (int j = 0; j < stat.sketchIterTime.size(); j++) {
			stat.avgSketchIterTime += stat.sketchIterTime.get(j);
		}
		stat.avgSketchIterTime /= stat.sketchIterTime.size();
 ...
 private static double norm(JavaSparkContext sc, final JavaRDD<org.apache.spark.mllib.linalg.Vector> vectors,
			...
			final Vector meanVector,final Matrix V, final Broadcast<Vector> br_ym_mahout, final double precision) {
		/************************** SSketchPCA PART *****************************/
		...
		for (int iter = 0; iter < q; iter++) {
			...
			newS = Math.round(newS * Math.pow(10,precision)) /  Math.pow(10,precision);//fixed point precision
			if (newS == S)
				break;
			else
				S = newS;
			...
			stat.sketchIterTime.add(totalTime/1000.0);
